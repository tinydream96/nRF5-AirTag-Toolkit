# nRF5 AirTag 统一刷写工具使用指南

本文档详细介绍了 `nrf5_airtag_flash.sh` 的使用方法。该工具集成了 J-Link 和 ST-Link 支持，并支持 Dynamic (动态密钥) 和 Static (静态密钥) 两种模式，旨在简化 nRF51822/nRF52832 等设备的量产刷写流程。
...
1. **多调试器支持**：自动识别与适配 **J-Link** (推荐) 和 **ST-Link**。
    * **自动回退机制**: J-Link 模式下，优先尝试标准 `nrfjprog`，若失败会自动切换到 `JLinkExe` 直连模式，确保 macOS 兼容性。
2. **多芯片支持**: 支持 nRF51822, nRF52832, nRF52810。
3. **双密钥模式**：
...
5. **参数配置**：支持自定义广播间隔 (Advertising Interval) 和自动增量。
...
* **`nrf5_airtag_flash.sh`**: 统一刷写脚本 (CLI)
* **`nrf5_airtag_web.py`**: Web 智控工具
